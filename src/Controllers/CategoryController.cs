...
namespace MarketMedia.src.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        {
            var category = await _iRepository.GetMainCategories();
            var n = new List<CategoryOutputDto>();
            foreach (var Item in category)
            {
                var m = new CategoryOutputDto();
        ...
        {
            var category = await _iRepository.GetSubCategories(parentCategoryId);
            var n = new List<CategoryOutputDto>();
            foreach (var Item in category)
            {
                var m = new CategoryOutputDto();
        ...
        #region PostCategory
        [HttpPost]
        public async Task<IActionResult> PostCategory([FromForm] CategoryInputDto inputDto)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutCategory(int id, [FromForm] Category inputDto)
        {
            var category = await _iRepository.GetCategory(id);
