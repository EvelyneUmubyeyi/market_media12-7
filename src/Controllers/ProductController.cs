...
;
        ...
        //Search Product
        [HttpGet("SearchProduct/{productName}/{categoryId}/{rating}")]
        public async Task<IActionResult> SearchProduct(string productName, int categoryId, decimal rating)
        {
            var products = await _iRepository.SearchProduct(productName, categoryId, rating);
            return Ok(products);
        }
        ...
        {
            var products = await _iRepository.GetProductInfo(productId);
            return Ok(products);
        }
