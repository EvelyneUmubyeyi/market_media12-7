...
;
        ...
        [HttpGet("BySeller/{productId}/{sellerId}/{categoryId}/{rating}")]
        public async Task<IActionResult> GetProductsBySellers(int productId, int sellerId, int categoryId, decimal rating)
        {
            var productSeller = await _iRepository.GetSellerProducts(productId, sellerId,categoryId, rating);
            return Ok(productSeller);
        }
        #endregion
        ...
        [HttpGet("SellersOfProduct/{productId}/{sellerId}/{rating}")]
        public async Task<IActionResult> GetAllSellersOfProduct(int productId, int sellerId, decimal rating)
        {
            var productSeller = await _iRepository.GetSellersOfProductInfo(productId,sellerId,rating);  
            return Ok(productSeller);
        }
        #endregion

        #region GetDeals(categoryId)
        [HttpGet("Deals/{categoryId}")]
        public async Task<IActionResult> GetDeals(int categoryId)
        {
            var deals = await _iRepository.GetDealsInfo(categoryId);
            return Ok(deals);
        }
        #endregion
