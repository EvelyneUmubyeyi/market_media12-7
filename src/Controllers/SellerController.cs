...
namespace MarketMedia.src.Controllers
{
    ...
    public class SellerController : Controller
    {
        ...
        #region GetSeller(nameId,categoryId,rating,productId)
        [HttpGet("{sellerId}/{categoryId}/{rating}/{productId}")]
        public async Task<IActionResult> GetSellerInfo(int sellerId, int categoryId, int rating, int productId)
        {
            var seller = await _iRepository.GetSellersInfo(sellerId, categoryId, rating, productId);
            return Ok(seller);
        }
        #endregion
