...
namespace MarketMedia.src.Services
{
    public class Repository:IRepository
    {
        ...
        {
            return await _Context.Categories.FromSqlRaw("select * from Lookup.Category where Lookup.Category.parentId IS NULL").ToListAsync();
        }

        public async Task<List<Category>> GetSubCategories(int parentCategoryId)
        {
            return await _Context.Categories.FromSqlRaw("select * from Lookup.Category where Lookup.Category.parentId="+ parentCategoryId).ToListAsync();
        }
        #endregion
        ...
        public async Task<List<SearchProductOutputDto>> SearchProduct(string productName, int categoryId, decimal rating)
        {
            return await _Context.SearchProductOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnSearchProduct]({0},{1},{2})", productName, categoryId,rating).ToListAsync();
        }

        public async Task<List<ProductDetailsOutputDto>> GetProductInfo(int productId)
        {
            return await _Context.ProductDetailsOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnGetProducts]({0})", productId).ToListAsync();
        }
        #endregion
        ...
        public async Task<List<DealsOutputDto>> GetDealsInfo(int categoryId)
        {
            return await _Context.DealsOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnDeals]({0})", categoryId).ToListAsync();
        }
        #endregion
        ...
        public async Task<List<SellerInfoOutputDto>> GetSellersInfo(int sellerId, int categoryId, int rating, int productId)
        {
            return await _Context.SellerInfoOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnGetSellers]({0},{1},{2},{3})", sellerId, categoryId, rating, productId).ToListAsync();
        }
        #endregion
        ...
        public async Task<List<SellersOfProductOutputDto>> GetSellersOfProductInfo(int productId, int sellerId, decimal rating)
        {
            return await _Context.SellersOfProductOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnGetProductSellers]({0},{1},{2})", productId,sellerId,rating
                ).ToListAsync();
        }

        public async Task<List<sellerProductsOutputDto>> GetSellerProducts(int productId, int sellerId, int categoryId, decimal rating)
        {
            return await _Context.sellerProductsOutputDtos.FromSqlRaw("SELECT * FROM [dbo].[fnGetSellerProducts]({0},{1},{2},{3})", productId, sellerId, 
                categoryId, rating).ToListAsync();
        }
